...
const CADENA_MANUAL  = 'Seleccione AYUDA si desea obtener más información sobre el problema surgido';
      CADENA_BORRADO = 'La sección seleccionada se encuentra referenciada en alguna analítica.';

procedure TSectionsView.CrearFiltro;
begin
...
procedure TSectionsView.PrepararQuery;
begin
   {$Message Warn 'La instrucción WITH es ofuscadora de código`'}
   with QFichero, ModifySql do begin
      Close;
      Clear;
      Add('UPDATE SECCION SET NOMBRE =:NOMBRE, ID_SECCION = :ID_SECCION');
      Add('WHERE ID_SECCION = :old_ID_SECCION');
   end;
   {$Message Warn 'La instrucción WITH es ofuscadora de código`'}
   with QFichero, SelectSQL do begin
      ...
      Close;
      Clear;
      Add('SELECT * FROM SECCION');

      if TbFiltro.FieldByName('BCODIGO').AsString <> '' then   begin
         add('WHERE ID_SECCION LIKE :ID_SECCION');
      end
      else
      ...
      if TbFiltro.FieldByName('BCODIGO').AsString <> '' then   begin
         params.byname('ID_SECCION').AsString :=
...
procedure TSectionsView.RefrescarBD;
begin
...
procedure TSectionsView.BtnNavAniadirClick(Sender: TObject);
begin
   ...
   try
      ...
      DatabaseError('No se ha podido insertar una nueva sección.' + #13 +
         CADENA_MANUAL);
   end;
   Modo(Self, Edita);
end;

procedure TSectionsView.BtnNavBorrarClick(Sender: TObject);
begin
   ...
   if not QFichero.IsEmpty then   begin
      if MessageDlg('¿Desea borrar esta sección?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
         try QFichero.Delete;
             ...
             RefrescarBD;
         except DatabaseError('No se ha podido borrar la sección seleccionada.' + #13 + CADENA_BORRADO);
         end;
      end;
   end;
end;

procedure TSectionsView.BtnNavImprimirClick(Sender: TObject);
begin
...
procedure TSectionsView.BtnEdtGuardarClick(Sender: TObject);
var
...
procedure TSectionsView.BtnEdtCancelarClick(Sender: TObject);
begin
   Perform(wm_NextDlgCtl, 0, 0);
...
procedure TSectionsView.RejillaDblClick(Sender: TObject);
begin
...
procedure TSectionsView.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TSectionsView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
   case key of
      VK_ESCAPE: begin
         key := 0;
...
procedure TSectionsView.RejillaTitleButtonClick(Sender: TObject; AFieldName: String);
begin
...
procedure TSectionsView.RejillaCalcTitleAttributes(Sender: TObject; AFieldName: String;
   AFont: TFont; ABrush: TBrush; var ATitleAlignment: TAlignment);
begin
...
procedure TSectionsView.BtnNavCerrarClick(Sender: TObject);
begin
   Close;
end;

procedure TSectionsView.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TSectionsView.fcImageBtnMinimizarClick(Sender: TObject);
begin
   windowState := wsminimized;
end;

procedure TSectionsView.fcImageBtnMouseEnter(Sender: TObject);
begin
   (Sender as TfcImageBtn).Image.LoadFromFile(gvDirImagenes + gcBtnBlanco);
end;

procedure TSectionsView.fcImageBtnMouseLeave(Sender: TObject);
begin
   (Sender as TfcImageBtn).Image.LoadFromFile(gvDirImagenes + gcBtn);
end;

procedure TSectionsView.LimpiarFiltro(Sender: TObject);
begin
   if not (TbFiltro.state in dseditmodes) then begin
      TbFiltro.edit;
   end;
...
procedure TSectionsView.VerTabla(Sender: TObject);
begin
   {$Message Warn 'La instrucción WITH es ofuscadora de código`'}
