...
function GetHash      (Stream  :TStream; Algorithm :THashAlgorithm):string; overload;
function GetHash      (Archivo :string ; Algorithm :THashAlgorithm):string; overload;
function GetStringHash(Str     :string ; Algorithm :THashAlgorithm):string;
...
begin
  ...
  if (not Success) then begin
     if GetLastError() = DWORD(NTE_BAD_KEYSET) then begin
        Success := CryptAcquireContext(@hProv, nil, nil, PROV_RSA_FULL, CRYPT_NEWKEYSET);
     end;
  end;

  if Success then begin
     if Algorithm = haMD5 then begin
        Algid   := CALG_MD5;
        Datalen := 16
     end
     else begin
     ...
     if CryptCreateHash(hProv, Algid, 0, 0, @hHash) then begin
        ...
        try
           while True do begin
              ...
              if BytesRead = 0 then begin
                 if CryptGetHashParam(hHash, HP_HASHVAL, @Data, @DataLen, 0) then begin
              ...
              if not CryptHashData(hHash, Buffer, BytesRead, 0) then Break;
           end;
        finally
          FreeMem(Buffer);
        end;
...
begin
   ...
   if FileExists(Archivo) then begin
      try
         ...
         try
            Result := GetHash(Stream, Algorithm);
         finally
            Stream.Free;
         end;
      except
