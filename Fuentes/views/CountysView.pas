...
type
  TCountysView = class(TccFormChild)
...
procedure TCountysView.AsignaDataSources;
begin
...
procedure TCountysView.FormCreate(Sender: TObject);
begin
...
procedure TCountysView.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   IProvincias.Free;
   HProvincias.Close;
   Action := caFree;
...
function  TCountysView.ValidData:Boolean;
begin
   Result := True;
...
procedure TCountysView.SetDefaultValues;
begin
...
function TCountysView.GetFirstCODIGO:string;
var Q :TSQLQuery;
begin
   Q := DMRef.CreateQuery(
          ['SELECT PROVINCIA                          ',
...
function TCountysView.GetLastCODIGO:string;
var Q :TSQLQuery;
begin
   Q := DMRef.CreateQuery(
          ['SELECT PROVINCIA                          ',
...
procedure TCountysView.MuestraError(prmErrorMessage :string);
begin
...
procedure TCountysView.NavegadorButtonAction(Sender: TObject; Button: TccNavResource);
//var Q :TIBQuery;
begin
   case Button of
...
procedure TCountysView.TimerTimer(Sender: TObject);
begin
   LabelMensaje.Caption := '';
   Timer.Enabled := False;
end;

procedure TCountysView.BtnEliminarClick(Sender: TObject);
...
procedure TCountysView.BtnEditarClick(Sender: TObject);
begin
...
procedure TCountysView.BtnNuevoClick(Sender: TObject);
begin
...
procedure TCountysView.BtnCancelarClick(Sender: TObject);
begin
...
procedure TCountysView.BtnAceptarClick(Sender: TObject);
begin
...
procedure TCountysView.GridDblClick(Sender: TObject);
begin
   BtnEditar.Click;
end;

function TCountysView.FormHelp(Command: Word; Data: Integer; var CallHelp: Boolean): Boolean;
begin
   Result := False;
   //ActivateHelp(Self, CallHelp);
end;

procedure TCountysView.FormShow(Sender: TObject);
begin
   //if not(HProvincias.State in dsEditModes) then
   //   NavegadorButtonAction(Self, nrLast);
end;

procedure TCountysView.WMNCHitTest(var Msg: TWMNCHitTest);
begin
...
procedure TCountysView.CreateParams(var Params: TCreateParams);
//const
