...
const CADENA_MANUAL  = 'Seleccione AYUDA si desea obtener más información sobre el problema surgido';
      CADENA_BORRADO = 'La delegación seleccionada se encuentra referenciada en alguna analítica.';

procedure TDelegationsView.CrearFiltro;
begin
...
procedure TDelegationsView.PrepararQuery;
begin
   {$Message Warn 'La instrucción WITH es ofuscadora de código`'}
...
procedure TDelegationsView.RefrescarBD;
begin
...
procedure TDelegationsView.BtnNavAniadirClick(Sender: TObject);
begin
...
procedure TDelegationsView.BtnNavBorrarClick(Sender: TObject);
begin
   ...
   if MessageDlg('¿Desea borrar esta delegación?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
      try QFichero.Delete;
          ...
          RefrescarBD;
      except DatabaseError('No se ha podido borrar la delegación seleccionada.' + #13 + CADENA_BORRADO);
      end;
   end;
end;

procedure TDelegationsView.BtnNavImprimirClick(Sender: TObject);
begin
...
procedure TDelegationsView.BtnEdtGuardarClick(Sender: TObject);
var
...
procedure TDelegationsView.BtnEdtCancelarClick(Sender: TObject);
begin
   Perform(wm_NextDlgCtl, 0, 0);
...
procedure TDelegationsView.RejillaDblClick(Sender: TObject);
begin
...
procedure TDelegationsView.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TDelegationsView.FormShow(Sender: TObject);
begin
   SetBounds(0, FormPrincipal.Panel1.Top + FormPrincipal.Panel1.Height + 1, Width, Height);
   FiltroBCodigo.SetFocus;
end;

procedure TDelegationsView.FormCreate(Sender: TObject);
begin
...
procedure TDelegationsView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
   case key of
      VK_ESCAPE: begin
         key := 0;
...
procedure TDelegationsView.RejillaTitleButtonClick(Sender: TObject; AFieldName: String);
begin
...
procedure TDelegationsView.RejillaCalcTitleAttributes(Sender: TObject; AFieldName: String;
   AFont: TFont; ABrush: TBrush; var ATitleAlignment: TAlignment);
begin
...
procedure TDelegationsView.BtnNavCerrarClick(Sender: TObject);
begin
   Close;
end;

procedure TDelegationsView.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TDelegationsView.fcImageBtnMinimizarClick(Sender: TObject);
begin
   windowState := wsminimized;
end;

procedure TDelegationsView.fcImageBtnMouseEnter(Sender: TObject);
begin
   (Sender as TfcImageBtn).Image.LoadFromFile(gvDirImagenes + gcBtnBlanco);
end;

procedure TDelegationsView.fcImageBtnMouseLeave(Sender: TObject);
begin
   (Sender as TfcImageBtn).Image.LoadFromFile(gvDirImagenes + gcBtn);
end;

procedure TDelegationsView.LimpiarFiltro(Sender: TObject);
begin
   if not (TbFiltro.state in dseditmodes) then begin
      TbFiltro.edit;
   end;
...
procedure TDelegationsView.VerTabla(Sender: TObject);
begin
   {$Message Warn 'La instrucción WITH es ofuscadora de código`'}
